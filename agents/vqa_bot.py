...
class VQABot:
    def __init__(self, model_tag, device='cuda', device_id=0, bit8=False, max_answer_tokens=-1):
        ...
        self.blip2_processor = Blip2Processor.from_pretrained(f"/home/czq/pretrained_models/{BLIP2ZOO[self.model_tag]}", local_files_only=True)
        if device == 'cpu':
            self.device = 'cpu'
            # self.blip2 = Blip2ForConditionalGeneration.from_pretrained(BLIP2ZOO[self.model_tag])
            self.blip2 = Blip2ForConditionalGeneration.from_pretrained(f"/home/czq/pretrained_models/{BLIP2ZOO[self.model_tag]}", local_files_only=True)
        else:
            ...
            #                                                            **dtype)
            self.blip2 = Blip2ForConditionalGeneration.from_pretrained(f"/home/czq/pretrained_models/{BLIP2ZOO[self.model_tag]}",
