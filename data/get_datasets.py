...
def get_name2label_label2name(dataset_name, dataset, train_classes):
    name2label = dict()

    if dataset_name == 'cifar100' or dataset_name == 'cifar100small':
        for k, v in dataset.class_to_idx.items():
            v = dataset.target_transform(v)
            if v in train_classes:
                name2label[k] = v
    else:
