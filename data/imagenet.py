...
from config import imagenet_root, imagenet_200_root, imagenet_10_root
...
def get_imagenet_10_datasets(train_transform, test_transform, train_classes=range(5),
                          prop_train_labels=0.8, split_train_val=False, seed=0, use_coarse_label=False, args=None):
    np.random.seed(seed)

    whole_training_set = ImageNetBase(root=os.path.join(imagenet_10_root, 'train'), transform=train_transform, use_coarse_label=use_coarse_label, class_num=10, data_path=imagenet_10_root)
    ...
    # Get test set for all classes
    test_dataset = ImageNetBase(root=os.path.join(imagenet_10_root, 'val'), transform=test_transform, use_coarse_label=use_coarse_label, class_num=10, data_path=imagenet_10_root)
