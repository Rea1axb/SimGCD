...
def test(model, test_loader, epoch, save_name, args):
    ...
    all_acc, old_acc, new_acc, ind, w = log_accs_from_preds(y_true=targets, y_pred=preds, mask=mask,
                                                    ...
                                                    args=args)
    coarse_ind, coarse_w, coarse_from_fine_acc, coarse_from_fine_ind, coarse_from_fine_w = 0., 0., 0., 0., 0.
    if args.use_coarse_label:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--coarse_label_num', type=int, default=-1)
    ...
    # --------------------
    if args.transform == 'double_transform':
        fine_train_transform, fine_test_transform = get_transform('imagenet', image_size=args.image_size, args=args)
        coarse_train_transform, coarse_test_transform = get_transform('gaussian_blur', image_size=args.image_size, args=args)
        train_transform = FineCoarseContrastiveLearningViewGenerator(fine_transform=fine_train_transform, coarse_transform=coarse_train_transform, n_views=args.n_views)
        test_transform = fine_test_transform
        args.double_transform = True
    else:
    ...
    projector = DoubleCoarseHead(in_dim=args.feat_dim, out_dim_fine=args.mlp_out_dim, out_dim_coarse=args.coarse_out_dim, mlp_nlayers=args.num_mlp_layers, double_transform=args.double_transform)
